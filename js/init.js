(function($, undefined){
  ...
  ShowPreloader();  
  $.ajax({ url: dir + "/get_paths.php", success: function(data){
  ...
  function closeModal(modal, button){
    $(modal).find(button).live('click', function(){
          var t = $(this), parent = t.parent(modal);
  ...
  function updatePath(){
    ...
    $('.point-edit').remove();
    $.ajax({ type: "POST", url: dir + "/set_path.php", data: form, success: function(data){
        if(data === "true") alert( "Участок обновлен!"); window.location.reload();
      }
    });
  }
  ...
  function uploadImage(){
    ...
    if($('input').is('#m-path-photo')){
      ...
      $.ajax({ url: dir + "/upload_photo.php", type: "POST", contentType: false, processData: false, data: fData, success: function(data){
